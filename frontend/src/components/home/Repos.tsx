...
export default function Repos() {
  ...
  return (
    <section id="code" className="pt-10 pb-24 px-6 relative overflow-hidden z-0">
      ...
      <div className="max-w-7xl mx-auto z-30 relative">
        ...
        <div className="flex flex-col items-center text-center mb-12 z-10 relative select-none">
          ...
          <div className="mt-5">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 px-5 py-2 rounded-full dreamcard text-xs font-semibold text-text-primary hover:text-accent-primary hover:border-accent-primary/40 transition-all duration-300 shadow-sm"
            >
              <svg className="w-4 h-4 fill-current text-accent-primary" viewBox="0 0 24 24" aria-hidden="true">
                <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.87 8.17 6.84 9.5.5.08.66-.23.66-.5v-1.69c-2.77.6-3.36-1.34-3.36-1.34-.46-1.16-1.11-1.47-1.11-1.47-.9-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.9 1.52 2.34 1.07 2.91.83.09-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.92 0-1.11.38-2 1.03-2.71-.1-.25-.45-1.29.1-2.64 0 0 .84-.27 2.75 1.02.79-.22 1.65-.33 2.5-.33.85 0 1.71.11 2.5.33 1.91-1.29 2.75-1.02 2.75-1.02.55 1.35.2 2.39.1 2.64.65.71 1.03 1.6 1.03 2.71 0 3.82-2.34 4.66-4.57 4.91.36.31.69.92.69 1.85V21c0 .27.16.59.67.5C19.14 20.16 22 16.42 22 12A10 10 0 0012 2z" />
              </svg>
              <span>Explore @jainiakhil on GitHub</span>
        ...
        {/* Repos Grid */}
        <motion.div layout="position" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <AnimatePresence mode="popLayout">
            {displayedRepos.map((repo, index) => (
              <motion.div
                ...
                transition={{ duration: 0.8, delay: index < 3 ? index * 0.12 : 0, ease: [0.25, 0.1, 0.25, 1.0] }}
        ...
        {/* View More Button - only if > 3 repos */}
        {repos.length > 3 && (
          <div className="flex justify-center mt-12 relative z-30">
            ...
            >
              {isExpanded ? "show fewer repositories ✦" : "view more repositories ✦"}
            </button>
          </div>
        )}
